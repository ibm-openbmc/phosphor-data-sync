    ...
    DataWatcher::processCreate(const EventInfo& receivedEventInfo)
{
    ...
    if ((std::get<EventMask>(receivedEventInfo) & IN_ISDIR) != 0)
    {
        ...
        {
            // Was monitoring exisitng parent path of the configured data path
            ...
            auto modifyWatchIfExpected = [this](const fs::path& entry) {
                if (_dataPathToWatch.string().starts_with(entry.string()))
                {
                    ...
                    // "/a/b/c/d". Here "d" is directory and is cfg path.
                    ...
                    // iteration in order to remove the watch for "c", need to
                    // iterate through map as WD of c is unknown.
                    if (auto parent =
    ...
    DataWatcher::processDeleteSelf(const EventInfo& receivedEventInfo)
{
    ...
    auto wd = _watchDescriptors.begin();
    while (wd != _watchDescriptors.end())
    {
        auto entry = wd;
        std::advance(wd, 1);
        if (entry->second.string().starts_with(deletedPath.string()))
        {
            removeWatch(entry->first);
        }
    }
